...
1. **Verify the import round-trip (highest value, no Pi needed).** Confirmed this session: ChoreOps' `validate_backup_json` **PASSES** the dev rig's raw storage file as-is (Store format v1; all 8 entity keys). The paste step accepts diagnostic, Store, **and raw storage** formats — so no special export is needed, the storage file itself is pasteable.
   ...
   - **Still unverified:** that a full round-trip reproduces every entity. Test by importing into the dev rig itself and diffing.
   - **Bonus:** writing JSON directly sets icons correctly with no picker — sidesteps §3's icon problem entirely.
2. **Build out the remaining content** on the dev rig (or straight into JSON), working from these sheets — all decisions pre-made:
...
- **⚠️ "3 SERVICES" vs 1 config entry — UNRESOLVED.** The dev rig's ChoreOps card reads "3 SERVICES" but `.storage/core.config_entries` holds only **one** `choreops` entry. Possibly leftovers from the 2026-08-14 failed-setup attempts (the "stuck in Google login loop" episode). **Investigate before the JSON import**, since import deletes/recreates the entry.
