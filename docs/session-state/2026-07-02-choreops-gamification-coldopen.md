...
- **✅ PI POWER RESOLVED + KIOSK FULLY WORKING (updated end of 2026-07-02 session).** The Pi is now on its **own official 27W USB-C brick** (ViewSonic powered separately, Pi micro-HDMI→ViewSonic HDMI for video). `vcgencmd get_throttled` reads clean `0x0`. The crash-loop (desktop-flash→white every ~4s) was the ViewSonic-over-USB-C brownout — see memory `pi-power-and-kiosk-login.md`. **The dashboard is showing and stable.** Kiosk login is now a persistent password session (see §2). Pi reachable at `ssh kitchencom` / `192.168.1.234`.
...
- **KIOSK BROUGHT FULLY ONLINE (long sub-arc, resolved):** fixed Pi power (27W brick, §0), fixed kiosk login. On the Pi, `/config/configuration.yaml` was edited then reverted to default password auth (a `configuration.yaml.bak.20260702` backup sits on the Pi). Tried trusted_networks auto-login — DOESN'T work for kiosks (always shows a user-picker form the browser can't submit). Final: default password auth + one-time "Keep me logged in" on the kiosk → refresh token stored for `http://localhost:8123/`, survives reboots. Verified dashboard showing + stable. Also cleared chromium web-state during debugging (Cache/Service Worker/Local Storage/IndexedDB) — benign. Full detail in memory `pi-power-and-kiosk-login.md`. **NOTE: the HA config change is Pi-side only (not in the repo — the choreops branch doesn't own `homeassistant/configuration.yaml` deploy). If reconciling later, the live Pi config = default auth, no auth_providers block.**
- **TOUCH still not working** — no touchscreen HID detected even after the user tried a USB-A↔USB-C cable to the ViewSonic. Likely a display-only ViewSonic model (need the model number to confirm). Mouse+keyboard work fine, so not blocking.
