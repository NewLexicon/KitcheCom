...
| `selectPlayableChildren(browseTree)` | *(new, M-13)* filter `browse_media` tree → playable image/video leaf items; skip `can_expand` dirs. **v1 flat, no recursion** (nested albums deferred — YAGNI) |
...
| `resolveConfig(rawConfig)` | *(new)* apply defaults; **clamp `photo_duration` to a sane floor (≥2s)** so a 0/negative value can't spin the loop |
| `shouldReResolve(resolvedAt, ttlSeconds, now)` | *(new, I-7 policy)* true if a resolved URL is stale/expiring (with safety margin). The expiry **decision** is tested; only the WS resolve call stays in glue |
...
LOOP (per displayed item):
  ...
  2. items = selectPlayableChildren(tree)                    [pure, M-13]
  3. if items empty → FALLBACK (gradient + clock)            [selectDisplayMode]
  4. i = nextMediaIndex(i, items.length)                     [pure, wrap]
  5. if shouldReResolve(item.resolvedAt, ttl, now)           [pure, I-7]
        → callWS(media_source/resolve_media); stamp resolvedAt   [glue side-effect]
  6. render:
